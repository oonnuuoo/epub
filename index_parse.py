...
output_csv = '../../' + args[1] + '/ocr/test.csv'
...
output_file = open(output_csv, 'w', encoding='utf-8-sig')
...
with open(input_xml, 'r', encoding='utf-8') as input_file:
    ...
    for line in input_file:
        ...
        if match1:
            ...
            print(x, y, width, height, string)
            output_file.write(img_name + ',' + img_width + ',' + img_height + ',' + x + ',' + y + ',' + width + ',' + height + ',' + string + '\n')
            continue
        else:
            print("skip")
            continue
